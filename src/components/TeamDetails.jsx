...
function useTeamPageData(teamId){
  ...
  const {response:articles,loading:teamArticlesLoading} =useTeamArticles(teamId);
    ...
    <section className='py-16'>
      <div className="container mx-auto text-center">
        <h1 className="text-4xl font-semibold text-center">
          {/* //if team exits show data(optional chaining) */}
          {team?.name}
        </h1>
        ...
        <ul className='mt-4 flex gap-4 justify-center'>
          {
            team?.championships.map((item)=>(<li key={item}>{item}</li>))
          }
        </ul>
        {
          team?(
